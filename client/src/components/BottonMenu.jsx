	...
	return (
		<div>
			<button onClick={toggleMenu}>
				<div
					className="flex p-2  items-center gap-3 rounded-3xl border border-gray-300 bg-white cursor-pointer hover:shadow-lg hover:border-gray-400 md:z-50"
					selected={selected}
					onClick={() => setSelected(!selected)}
				>
					<img src={menu} />
					<img className="md:w-8 w-10 rounded-full" src={user ? photoPerson : usuario} />{" "}
					...
					<div className="p-2 flex flex-col m-0 ml-3 mr-20   gap-4">
						{user ? ( // Verifica si el usuario está autenticado
							...
							<>
								<Link to="/register">
									<p
										className="flex whitespace-nowrap m-0 text-base cursor-pointer hover:opacity-70"
										onClick={() => openModal(1)}
									>
										Registrate
									</p>
								...
								<Link to="/register">
									<p
										className="flex whitespace-nowrap m-0 text-base cursor-pointer hover:opacity-70 border-b mb-2 pb-5"
										onClick={() => openModal(2)}
									>
										Inicia Sesión
									</p>
